...
artea_ab = read.csv("521703-XLS-ENG.xlsx - AB_test.csv")9
...
df = artea_ab %>% group_by(channel_acq,test_coupon )  %>%
  summarise(Revenue = sum(revenue_after),
            Transactions = sum(trans_after),
            .groups = 'drop')
...
colnames(df)[2] <- "Coupon"
colnames(df)[1] <- "Channel"

require(gridExtra)
plot1 = ggplot(df, aes(fill=Channel, y=Revenue, x=Coupon)) + geom_bar(position="dodge", stat="identity")
plot2 = ggplot(df, aes(fill=Channel, y=Transactions, x=Coupon)) + geom_bar(position="dodge", stat="identity")
